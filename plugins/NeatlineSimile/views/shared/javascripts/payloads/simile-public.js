(function (a) {
	...
	function x(a) {
		...
		if (!a._d) {
			...
			e[4] += ~~((a._tzm || 0) % 60),
			c = new Date(0),
			a._useUTC ? (c.setUTCFullYear(e[0], e[1], e[2]), c.setUTCHours(e[3], e[4], e[5], e[6])) : (c.setFullYear(e[0], e[1], e[2]), c.setHours(e[3], e[4], e[5], e[6])),
			a._d = c
		}
	}
	function y(a) {
	...
	for (mb.DDDD = b(mb.DDD, 3), g(d.prototype, {
		...
		return a instanceof f
			...
			isValid : function () {
				return null == this._isValid && (this._isValid = this._a ? !l(this._a, (this._isUTC ? L.utc(this._a) : L(this._a)).toArray()) : !isNaN(this._d.getTime())),
				!!this._isValid
			},
			invalidAt : function () {
				var a,
			...
			add : function (a, b) {
				var c;
				return c = "string" == typeof a ? L.duration(+b, a) : L.duration(a, b),
				j(this, c, 1),
				this
			},
			...
			diff : function (a, b, c) {
				...
				return b = m(b),
				"year" === b || "month" === b ? (d = 432e5 * (this.daysInMonth() + f.daysInMonth()), e = 12 * (this.year() - f.year()) + (this.month() - f.month()), e += (this - L(this).startOf("month") - (f - L(f).startOf("month"))) / d, e -= 6e4 * (this.zone() - L(this).startOf("month").zone() - (f.zone() - L(f).startOf("month").zone())) / d, "year" === b && (e /= 12)) : (d = this - f, e = "second" === b ? d / 1e3 : "minute" === b ? d / 6e4 : "hour" === b ? d / 36e5 : "day" === b ? (d - g) / 864e5 : "week" === b ? (d - g) / 6048e5 : d),
				c ? e : h(e)
			},
			from : function (a, b) {
	...
	define : function (a) {
		a.View = Neatline.Shared.Widget.View.extend({
				...
				focusByModel : function (a) {
					var b = moment(String(a.get("start_date"))),
					c = moment(String(a.get("end_date")));
					if (b.isValid() && c.isValid()) {
						var d = b.add("milliseconds", (c - b) / 2);
						this.setCenterDate(d.toDate().toISOString())
					} else
						b.isValid() && this.setCenterDate(b.toDate().toISOString())
				},
				setFilter : function () {
